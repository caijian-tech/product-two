document.addEventListener('DOMContentLoaded', () => {
    ...
    // Fetch 7-day weather forecast for Seoul
    async function fetchWeather() {
        const weatherContent = document.getElementById('weather-content');
        try {
            const response = await fetch('https://api.open-meteo.com/v1/forecast?latitude=37.5665&longitude=126.9780&daily=weathercode,temperature_2m_max,temperature_2m_min&timezone=Asia%2FSeoul');
            ...
            if (weatherContent) {
                weatherContent.innerHTML = '<div class="loading-spinner">날씨 로드 오류</div>';
            }
        }
    }
        ...
        for (let i = 0; i < daily.time.length; i++) {
            ...
            dayElement.innerHTML = `
                <div class="day-info">
                    ...
                    <span class="day-desc">${weatherInfo.desc}</span>
                </div>
                <div class="day-temp-icon">
                    <span class="day-icon" title="${weatherInfo.desc}">${weatherInfo.icon}</span>
                    <span class="day-temp">${maxTemp}° / ${minTemp}°</span>
                </div>
            `;
            weatherContent.appendChild(dayElement);
        }
    }

    function getWeatherInfo(code) {
        switch (code) {
            case 0: return { icon: '☀️', desc: '맑음' };
            case 1: case 2: case 3: return { icon: '☁️', desc: '구름조금' };
            case 45: case 48: return { icon: '🌫️', desc: '안개' };
            case 51: case 53: case 55: return { icon: '🌦️', desc: '이슬비' };
            case 61: case 63: case 65: return { icon: '🌧️', desc: '비' };
            case 71: case 73: case 75: return { icon: '❄️', desc: '눈' };
            case 77: return { icon: '🌨️', desc: '싸락눈' };
            case 80: case 81: case 82: return { icon: '🌦️', desc: '소나기' };
            case 85: case 86: return { icon: '🌨️', desc: '눈소나기' };
            case 95: case 96: case 99: return { icon: '⛈️', desc: '뇌우' };
            default: return { icon: '🌡️', desc: '정보없음' };
        }
    }
